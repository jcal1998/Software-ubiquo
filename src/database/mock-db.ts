...
export const getEosinophiliaCasesInWindow = async (start: Date, end: Date): Promise<EosinophiliaCase[]> => {
    console.log(`DB: Fetching eosinophilia cases between ${start.toISOString()} and ${end.toISOString()}`);
    return dbEosinophiliaCases.filter(c => c.test_date >= start && c.test_date <= end);
};

export const getTotalTestsInArea = async (lat: number, lon: number, radiusKm: number, start: Date, end: Date): Promise<number> => {
...
export const getBaselineForRegion = async (municipality_id: string, month_year: string): Promise<RegionalBaseline | null> => {
    console.log(`DB: Fetching baseline for region ${municipality_id} and period ${month_year}`);
    const baseline = dbRegionalBaselines.find(b => b.region_id === municipality_id && b.month_year === month_year);
